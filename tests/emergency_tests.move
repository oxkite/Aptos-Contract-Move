#[test_only]
module defiralia_staking::emergency_tests {
    use std::option;
    use std::string;
    ...
    use defiralia_staking::stake_test_helpers::{new_account_with_stake_coins, mint_default_coin, RewardCoin, StakeCoin, new_account};
    ...
    fun test_cannot_stake_with_emergency() {
        ...
        stake::enable_emergency<StakeCoin, RewardCoin>(&emergency_admin, @defiralia_staking);
    ...
    fun test_cannot_unstake_with_emergency() {
        ...
        let coins = stake::unstake<StakeCoin, RewardCoin>(&alice_acc, @defiralia_staking, 100);
    ...
    fun test_cannot_harvest_with_emergency() {
        ...
        let reward_coins = stake::harvest<StakeCoin, RewardCoin>(&alice_acc, @defiralia_staking);
    ...
    fun test_cannot_enable_emergency_with_non_admin_account() {
        ...
        stake::enable_emergency<StakeCoin, RewardCoin>(&alice_acc, @defiralia_staking);
    }
    ...
    fun test_unstake_everything_in_case_of_emergency() {
        ...
        let coins =
            coin::withdraw<StakeCoin>(&alice_acc, 1 * ONE_COIN);
        stake::stake<StakeCoin, RewardCoin>(&alice_acc, @defiralia_staking, coins);
        assert!(stake::get_user_stake<StakeCoin, RewardCoin>(@defiralia_staking, @alice) == 1 * ONE_COIN, 1);

        stake::enable_emergency<StakeCoin, RewardCoin>(&emergency_admin, @defiralia_staking);

        let (coins, nft) = stake::emergency_unstake<StakeCoin, RewardCoin>(&alice_acc, @defiralia_staking);
        assert!(coin::value(&coins) == 1 * ONE_COIN, 2);
        assert!(option::is_none(&nft), 1);
        option::destroy_none(nft);
        coin::deposit(@alice, coins);

        assert!(!stake::stake_exists<StakeCoin, RewardCoin>(@defiralia_staking, @alice), 3);
    }
    ...
    fun test_emergency_is_local_to_a_pool() {
        ...
        let duration = 12345;
        stake::register_pool<StakeCoin, RewardCoin>(&defiralia_staking, reward_coins_1, duration, option::none());
        stake::register_pool<RewardCoin, StakeCoin>(&defiralia_staking, reward_coins_2, duration, option::none());

        stake::enable_emergency<RewardCoin, StakeCoin>(&emergency_admin, @defiralia_staking);

        let coins =
            coin::withdraw<StakeCoin>(&alice_acc, 1 * ONE_COIN);
        stake::stake<StakeCoin, RewardCoin>(&alice_acc, @defiralia_staking, coins);
        assert!(stake::get_user_stake<StakeCoin, RewardCoin>(@defiralia_staking, @alice) == 1 * ONE_COIN, 3);
    }
